        ...
        <>
          <div className="kpi-grid">
            <div className="kpi accent">
              ...
              <span className="kpi-sub">@ {volume.toLocaleString()} units/yr</span>
            </div>
            <div className="kpi">
          ...
          <div className="row-2" style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 16 }}>
            ...
            <div className="card">
              ...
              <div style={{ marginTop: 8, fontSize: 11.5, color: "var(--ink-3)" }}>
                Each tile is a part, sized by {metric === "cost" ? "total cost" : "total weight"} contribution (qty × {metric === "cost" ? "unit cost" : "unit weight"}). Click a tile to open it.
...
function VolumeChart({ tiers, selected }) {
  ...
  const max = Math.max(1, ...tiers.map((t) => t.total));
  const xs = tiers.map((_, i) => pad.l + (tiers.length === 1 ? iw / 2 : (i / (tiers.length - 1)) * iw));
  const ys = tiers.map((t) => pad.t + ih - (t.total / max) * ih);
  const path = tiers.map((_, i) => `${i === 0 ? "M" : "L"} ${xs[i]} ${ys[i]}`).join(" ");
  return (
    <svg viewBox={`0 0 ${w} ${h}`} style={{ width: "100%", height: "auto", display: "block" }}>
      {[0, 0.5, 1].map((g) => {
        const y = pad.t + ih - g * ih;
        return <g key={g}><line x1={pad.l} x2={w - pad.r} y1={y} y2={y} stroke="var(--hair)" /><text x={pad.l - 8} y={y + 3} textAnchor="end" fontFamily="var(--font-mono)" fontSize="9.5" fill="var(--ink-3)">€{((max * g) / 1000).toFixed(1)}k</text></g>;
      })}
